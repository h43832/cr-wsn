...
import javax.swing.*;
import java.io.*;
public class WSNApplication_IAQ extends WSNApplication implements Runnable{
  WSN wsn;
  Vector waitV=new Vector();
  boolean isSleep=false;
  long longWaitTime=31536000000L;
  Thread myThread;
  byte rsp_Array[]=new byte[1000];
    ...
    while(true){
        while(waitV.size()>0){
          ...
          for(int i=0;i<b0.length;i++){
            if((b0[i] & 0xF0) == 0xA0) rsp_len=0;
            else {rsp_Array[rsp_len]=b0[i]; rsp_len++;}
            if(rsp_len>=10){
                if(!(rsp_Array[4]==0x41 || rsp_Array[4]==0x42 || rsp_Array[4]==0x50)){rsp_len=0; break;}
                if(!(Byte.toString(rsp_Array[2]).equals(config[1]) || Byte.toString(rsp_Array[2]).equals(config[2]) || Byte.toString(rsp_Array[2]).equals(config[3]))) {rsp_len=0; break;}
                xValue = "";
                

                int raw= unsignedByte(rsp_Array[5]) * 256 + unsignedByte(rsp_Array[6]);
                Co2 = raw;
                ...
                switch(rsp_Array[4]){
                    case 0x41:
                        if(((int)rsp_Array[2])==Integer.parseInt(config[1])) {jLabel23.setText(""+RH); onoff2[1]=true;}
                        if(((int)rsp_Array[2])==Integer.parseInt(config[2])) {jLabel24.setText(""+RH); onoff2[2]=true;}
                        if(((int)rsp_Array[2])==Integer.parseInt(config[3])) {jLabel25.setText(""+RH); onoff2[3]=true;}
                        xValue = " RH : " + RH;
                        break;
                    case 0x42:
                        if(((int)rsp_Array[2])==Integer.parseInt(config[1])) {jLabel20.setText(""+Temp); onoff2[1]=true;}
                        if(((int)rsp_Array[2])==Integer.parseInt(config[2])) {jLabel21.setText(""+Temp); onoff2[2]=true;}
                        if(((int)rsp_Array[2])==Integer.parseInt(config[3])) {jLabel22.setText(""+Temp); onoff2[3]=true;}
                        xValue = " Temp : " + Temp;
                        break;
                    case 0x50:
                        if(((int)rsp_Array[2])==Integer.parseInt(config[1])) {jLabel10.setText(""+Co2); onoff2[1]=true;}
                        if(((int)rsp_Array[2])==Integer.parseInt(config[2])) {jLabel26.setText(""+Co2); onoff2[2]=true;}
                        if(((int)rsp_Array[2])==Integer.parseInt(config[3])) {jLabel27.setText(""+Co2); onoff2[3]=true;}
                        xValue = " Co2 : " + Co2;
                        break;
                 }
                if(xValue.length()>0) {
                  jTextArea1.append("["+wsn.formatter3.format(new Date())+"] ID = "+rsp_Array[2]+" "+xValue+"\r\n");
                }
                rsp_len = 0;
              }
            }
